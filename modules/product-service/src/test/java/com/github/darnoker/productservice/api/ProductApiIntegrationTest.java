...
class ProductApiIntegrationTest extends BaseApiTest {
    ...
    void createsAndFindsBook() throws Exception {
        ...
        MvcResult result = mvc.perform(post("/products").with(jwt().authorities(new SimpleGrantedAuthority("ROLE_ADMIN")))
    ...
    void listsProductsFilteredByType() throws Exception {
        ...
        mvc.perform(post("/products").with(jwt().authorities(new SimpleGrantedAuthority("ROLE_ADMIN")))
                        .contentType("application/json").content(jsonMapper.writeValueAsString(request)))
    ...
    void listsProductsFilteredByIds() throws Exception {
        ...
        CreateProductRequest secondRequest = new CreateProductRequest().name("Steel Longsword").description("Two handed sword").price(new BigDecimal("299.99"))
                .productType(ProductType.SWORD).details(Map.of("damage", 42, "weight", 3.5, "length", 115.0, "material", "STEEL"));
        ProductDTO first = jsonMapper.readValue(mvc.perform(post("/products").with(jwt().authorities(new SimpleGrantedAuthority("ROLE_ADMIN")))
                        .contentType("application/json").content(jsonMapper.writeValueAsString(firstRequest)))
                .andExpect(status().isCreated()).andReturn().getResponse().getContentAsString(), ProductDTO.class);
        ProductDTO second = jsonMapper.readValue(mvc.perform(post("/products").with(jwt().authorities(new SimpleGrantedAuthority("ROLE_ADMIN")))
                        .contentType("application/json").content(jsonMapper.writeValueAsString(secondRequest)))
                .andExpect(status().isCreated()).andReturn().getResponse().getContentAsString(), ProductDTO.class);
