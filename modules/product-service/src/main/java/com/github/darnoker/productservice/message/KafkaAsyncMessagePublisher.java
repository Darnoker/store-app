...
class KafkaAsyncMessagePublisher implements AsyncMessagePublisher {
    ...
    public CompletionStage<Void> publish(OutboundMessage message) {
        EventEnvelope<JsonNode> envelope = new EventEnvelope<>(message.id(), message.aggregateId(), message.eventType(),
                message.occurredAt(), EVENT_SCHEMA_VERSION, objectMapper.readTree(message.payload()));
        return kafkaTemplate.send(message.destination(), message.aggregateId().toString(), objectMapper.writeValueAsString(envelope))
                .thenApply(result -> {
                    log.info("Sent outbox message {} to {}", message.id(), result.getProducerRecord().topic());
                    return null;
                });
    }
}
